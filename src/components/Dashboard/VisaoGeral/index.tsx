...
const Transactions: React.FC = () => {
  ...
  useEffect(() => {
    if (contas) dispatch(set_transaction_data({ accounts: contas }))
  }, [dispatch, contas])
  ...
  useEffect(() => {
    ...
    const getDashboardValues = async () => {
      try {
        ...
        const dateFormated =
          newD.getFullYear() + "-" + (newD.getMonth() + 1) + "-" + newD.getDate()
        ...
        const result = await api.get(
          `/dashboard?fim=${formatDate(dateFormated)}&inicio=${formatDate(
            newDateFormated
          )}&login=${user?.login}`,
          {
            headers: {
  ...
  if (loaded)
    return (
      <div>
        ...
        <div>
          <p>
            Escolhe a quantidade de <strong>meses</strong> para o filtro:{" "}
          </p>
          <input
            type="number"
            min={1}
